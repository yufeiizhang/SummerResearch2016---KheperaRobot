...
% input data:
% Robot Location & Orientation:           DataSet
% Sensor Reading:                         Values



%% Position
r1=[DataSet(counter,1); DataSet(counter,3)];
r2=[DataSet(counter,1+6); DataSet(counter,3+6)];
r3=[DataSet(counter,1+12); DataSet(counter,3+12)];
r4=[DataSet(counter,1+18); DataSet(counter,3+18)];
...
rk1=[DataSet(counter-1,1); DataSet(counter-1,3)];
rk2=[DataSet(counter-1,1+6); DataSet(counter-1,3+6)];
rk3=[DataSet(counter-1,1+12); DataSet(counter-1,3+12)];
rk4=[DataSet(counter-1,1+18); DataSet(counter-1,3+18)];
...
if isnan(s.x)
    ...
    px = (Values(counter,1) - Values(counter,3))/2;
    % py= (fun_inter(rc(1),rc(2)+1,u)-fun_inter(rc(1),rc(2),u))/2;
    py = (Values(counter,4) - Values(counter,2))/2;
    % py = (u(x,y+1)-u(x,y))/2;
...
Laplaciansum=[Laplaciansum,Laplacian];
...
% it seems the data are invert



%% UPDATE FIGURE
plot(0.538,0,'kx',rc(1),rc(2),'co',r1(1),r1(2),'g*',r2(1),r2(2),'b*',r3(1),r3(2),'k*',r4(1),r4(2),'r*','MarkerSize',10,'MarkerFaceColor','b')
...
%% Interface.!
xTar = [r1(1),r2(1),r3(1),r4(1)];
zTar = [r1(2),r2(2),r3(2),r4(2)];
